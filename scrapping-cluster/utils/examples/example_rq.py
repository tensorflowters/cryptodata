...
def main():
    ...
    parser.add_argument(
        "-r", "--redis-host", action="store", default="scdev", help="The Redis host ip"
    )
    parser.add_argument(
        "-rp", "--redis-port", action="store", default="6379", help="The Redis port"
    )
    parser.add_argument(
        "-rP",
        "--redis-password",
    ...
    group.add_argument("-s", "--stack", action="store_true", help="Use a RedisStack")
    group.add_argument(
        "-p", "--priority", action="store_true", help="Use a RedisPriorityQueue"
    )
