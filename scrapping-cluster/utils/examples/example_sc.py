...
# set up arg parser
parser = argparse.ArgumentParser(description="Example key press stats collector.\n")
parser.add_argument(
    "-rw",
    "--rolling-window",
    action="store_true",
    required=False,
    help="Use a RollingTimeWindow counter",
    default=False,
)
parser.add_argument(
...
parser.add_argument(
    "-P", "--redis-password", action="store", default=None, help="The Redis password"
)
...
if args["rolling_window"]:
    ...
    counter = StatsCollector.get_time_window(
        host=args["redis_host"],
        port=args["redis_port"],
        password=args["redis_password"],
        window=the_window,
