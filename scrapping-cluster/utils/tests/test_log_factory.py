...
class TestLogFactory(TestCase):
    def setUp(self):
        self.logger = LogFactory.get_instance(
            name="test", dir="./", level="DEBUG", propagate=True
        )
...
class TestLogJSONFile(TestCase):
    def setUp(self):
        self.logger = LogObject(
            name="test", json=True, dir=".", level="INFO", stdout=False, file="test.log"
        )
        self.test_file = "./test"
...
class TestLogCallbacks(TestCase):
    ...
    def test_register_callback(self):
        ...
        callbacks = [
            cb for cb, criteria in self.logger.cb_handler.callbacks["CRITICAL"]
        ]
