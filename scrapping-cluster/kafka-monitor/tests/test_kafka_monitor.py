...
class TestKafkaMonitor(TestCase):
    ...
    def test_load_plugins(self):
        ...
        self.kafka_monitor.settings["PLUGINS"][
            "plugins.scraper_handler.ScraperHandler"
        ] = None
        self.kafka_monitor._load_plugins()
        self.assertEqual(list(self.kafka_monitor.plugins_dict.keys()), assert_keys)
        self.kafka_monitor.settings["PLUGINS"][
        ...
        self.kafka_monitor.settings["PLUGINS"]["plugins.crazy_class.CrazyHandler"] = 300
        self.assertRaises(ImportError, self.kafka_monitor._load_plugins)
        del self.kafka_monitor.settings["PLUGINS"]["plugins.crazy_class.CrazyHandler"]
    ...
    def test_load_stats_total(self):
        ...
        self.assertEqual(
            list(self.kafka_monitor.stats_dict["total"].keys()), ["lifetime"]
        )
        self.assertEqual(
            list(self.kafka_monitor.stats_dict["fail"].keys()), ["lifetime"]
        )
    ...
    def test_load_stats_plugins(self):
        ...
        for key in self.kafka_monitor.plugins_dict:
            plugin_name = self.kafka_monitor.plugins_dict[key][
                "instance"
            ].__class__.__name__
            self.assertEqual(
        ...
        self.assertEqual(
            sorted(self.kafka_monitor.stats_dict["plugins"].keys()), sorted(defaults)
        )
        ...
        for key in self.kafka_monitor.plugins_dict:
            ...
            self.assertEqual(
                sorted(
                    [
                        str(x)
                        for x in self.kafka_monitor.stats_dict["plugins"][
                            plugin_name
                        ].keys()
                    ]
                ),
        ...
        for plugin_key in self.kafka_monitor.stats_dict["plugins"]:
            ...
            for time_key in self.kafka_monitor.stats_dict["plugins"][plugin_key]:
                if time_key == 0:
                    ...
                    self.assertEqual(
                        self.kafka_monitor.stats_dict["plugins"][plugin_key][
                            time_key
                        ].key,
                        "{k}:{t}".format(k=k1, t=time_key),
                    )
    ...
    def test_process_messages(self):
        ...
        self.kafka_monitor.consumer = MagicMock(side_effect=OffsetOutOfRangeError("1"))
        try:
        ...
        self.kafka_monitor._load_plugins()
        self.kafka_monitor.validator = self.kafka_monitor.extend_with_default(
            Draft4Validator
        )
        list(self.kafka_monitor.plugins_dict.items())[0][1][
            "instance"
        ].handle = MagicMock(side_effect=AssertionError("scrape"))
        list(self.kafka_monitor.plugins_dict.items())[1][1][
            "instance"
        ].handle = MagicMock(side_effect=AssertionError("action"))

        #  test that handler function is called for the scraper
        message_string = '{"url":"www.stuff.com","crawlid":"1234",' '"appid":"testapp"}'
        m.value = message_string
        messages = [m]
        self.kafka_monitor.consumer.__iter__.return_value = messages
        try:
