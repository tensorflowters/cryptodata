...
class TestLinkSpider(TestCase):
    ...
    def evaluate(self, meta_object, text, expected_raw, expected_requests):
        request = Request(url="http://www.drudgereport.com", meta=meta_object)
        response = HtmlResponse(
            "drudge.url", body=text, request=request, encoding="utf8"
        )
