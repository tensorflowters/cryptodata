...
class DistributedScheduler(object):
    ...
    def update_ipaddress(self):
        ...
        try:
            obj = urllib.request.urlopen(
                new_settings.get("PUBLIC_IP_URL", "http://ip.42.pl/raw")
            )
    ...
    def report_self(self):
        ...
        """
        self.logger.debug(
            "Reporting self id: {extra}".format(extra={"uuid": self.my_uuid})
        )
    ...
    def from_settings(cls, settings):
        server = redis.Redis(
            host=new_settings.get("REDIS_HOST"),
        ...
        global_page_per_domain_limit = new_settings.get(
            "GLOBAL_PAGE_PER_DOMAIN_LIMIT", None
        )
        global_page_per_domain_limit_timeout = new_settings.get(
