...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scrapy Cluster Migration "
    ...
    parser.add_argument(
        ...
        action="store",
        required=False,
        help="The output Redis host ip, defaults to input",
        default=None,
    )
    parser.add_argument(
        "-op",
    ...
    parser.add_argument(
        ...
        action="store",
        type=float,
        help="The current cluster version",
        required=True,
        choices=[1.0, 1.1],
    )
    parser.add_argument(
        ...
        action="store",
        default=1.2,
        help="The desired cluster version",
        required=True,
        choices=[1.1, 1.2],
        type=float,
    )
    ...
    if irh == orh and ird == ord and irp == orp and args["end_version"] >= 1.2:
        print(
            "Warning! Exact same Redis settings detected, migration will "
                ...
                if len(elements) == 2:
                    ...
                    for item in i_redis_conn.zscan_iter(queue):
                        ...
                        key = "{sid}:{dom}.{suf}:queue".format(
                            sid=item["spiderid"], dom=ex_res.domain, suf=ex_res.suffix
                        )
                    ...
                    if new_count == old_count:
                        ...
                        vprint(
                            "Unknown error when migrating requests {o}/{n}".format(
    ...
    vprint(
        "Upgraded cluster from "
        + str(args["start_version"])
        + " to "
