...
bro_parser = subparsers.add_parser(
    "bro", help="Prints only the brother's name", parents=[base_parser]
)
bro_parser.add_argument(
    "-b", "--brother", action="store", required=True, help="The brother's name"
)

# subcommand 2, requires name of sister and optionally mom
fam_parser = subparsers.add_parser(
    "fam", help="Prints mom and sister's name", parents=[base_parser]
)
fam_parser.add_argument(
    "-s", "--sister", action="store", required=True, help="The sister's name"
)
fam_parser.add_argument(
    "-m",
    "--mom",
...
if args["command"] == "bro":
    ...
    print(
        "Your sister's name is " + args["sister"] + " and you call your "
        "Mother '" + args["mom"] + "'"
    )
