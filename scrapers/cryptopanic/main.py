...
try:
    ...
    target_main_section = driver.find_element(
        By.XPATH, ".//div[@class='app-main-pane']"
    )
    ...
    for news_row in target_news_rows:
        ...
        source_domain_element = news_row.find_element(
            By.XPATH,
            ".//div[@class='news-cells']//a[@class='news-cell nc-title']//span[@class='si-source-name']//span[@class='si-source-domain']",
        )
        ...
        if news_row.find_elements(
            By.XPATH, ".//div[@class='news-cells']//div[@class='news-cell nc-currency']"
        ):
            ...
            if len(currencies_element) > 0:
                for currency in currencies_element:
                    if (
                        currency != None
                        and currency.text != ""
                        and currency.text not in currencies
                    ):
